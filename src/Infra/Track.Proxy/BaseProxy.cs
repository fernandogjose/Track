...
namespace Track.Proxy
{
    ...
    public class BaseProxy
    {
        ...
        private async Task LogResponse(string url, string response, string method, string tokenApi)
        {
            LogRequest logRequest = new LogRequest
            {
                StatusCode = StatusCode.Info.ToString(),
                Message = $"Url: {url} - Response: {response} - Token: {tokenApi}",
                Method = method,
                NamespaceClass = "Track.Proxy.BaseProxy",
                LogDate = DateTime.Now
            };
        ...
        public async Task<BaseResponse> HttpPostAsync(string url, string request, AuthenticationResponse authenticationResponse, string method)
        {
            ...
            var response = new BaseResponse() { Contents = contents, HttpResponseMessage = result };
            ...
            await LogResponse(url, contents, method, authenticationResponse == null ? "GetToken" : authenticationResponse.Token);

            return response;
        }
        public async Task<BaseResponse> HttpPutAsync(string url, string request, AuthenticationResponse authenticationResponse, string method)
        {
